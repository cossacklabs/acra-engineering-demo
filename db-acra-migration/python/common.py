...
def register_common_cli_params(parser):
    ...
    parser.add_argument('--port', type=int,
                        default=get_default('port', 5432),
                        help='Port of database or AcraServer')
    parser.add_argument('--host', type=str,
                        default=get_default('host', 'localhost'),
                        help='Host of database or AcraServer')
    parser.add_argument('--print', action='store_true',
                        default=get_default('print', False),
                        help='Print data')
    parser.add_argument('--generate', action='store_true',
                        default=get_default('generate', False),
                        help='Generate data')
    parser.add_argument('--migrate', action='store_true',
                        default=get_default('migrate', False),
                        help='Migrate data')
