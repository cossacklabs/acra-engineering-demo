...
Please do not forget to add a temporary entry to the hosts file:
    ...
    echo 'SERVER_IP www.djangoproject.dev' >> $ETCHOSTS_PREFIX/etc/hosts

    where SERVER_IP - IP address of server with running Acra Engineering Demo.
"
    echo '
...
acraengdemo_launch_project_django() {
    ...
    export COSSACKLABS_DJANGO_VCS_URL='https://github.com/cossacklabs/djangoproject.com'
    export COSSACKLABS_DJANGO_VCS_BRANCH='master'
    acraengdemo_cmd \
        "git clone --depth 1 $COSSACKLABS_DJANGO_VCS_URL" \
        "Cloning djangoproject.com"
    acraengdemo_cmd "cp -r $BASEDIR/djangoproject.com $PROJECT_DIR/"
    export COSSACKLABS_DJANGO_VCS_REF=$(git -C ./djangoproject.com/ rev-parse --verify HEAD)

    COSSACKLABS_ACRAENGDEMO_VCS_URL='https://github.com/cossacklabs/acra-engineering-demo'
    COSSACKLABS_ACRAENGDEMO_VCS_BRANCH='master'
    acraengdemo_cmd \
        "git clone --depth 1 $COSSACKLABS_ACRAENGDEMO_VCS_URL" \
