...
RUN mkdir /tmp/acra
RUN git clone $VCS_URL /tmp/acra \
    && cd /tmp/acra \
    && git checkout $VCS_REF \
    && cd /

RUN mkdir /app \
    && mv -v /tmp/acra/examples/python/* /app/
...
RUN mkdir /certs
COPY ./_common/cockroachdb/certs/ /certs/

RUN chmod 0600 -R /certs/

COPY ./cockroachdb/entry.sh /entry.sh
